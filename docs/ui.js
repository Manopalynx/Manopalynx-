...
// The two upkeep dials, measured over 400 games a cell at 96 circuits on Sam's
// table before any of these labels were written.
//
// BUILDINGS IS THE STRONG ONE and it is NOT monotonic, which is the finding
// worth carrying: conquest runs 83% / 85% / 75% / 61% / 50% across 0x to 2x, so
// the peak is at a HALF and zero is slightly below it. Free buildings do not
// mean more absorption -- absorption needs somebody bankrupted, and at 0x
// nothing bankrupts anybody except rent. What 0x does do is punish the player
// hardest of the five: the human's win rate falls to 34% against 44% at 1x,
// because the opponents build everything and charge for it. "No upkeep" sounds
// like a kindness and is the least kind setting on the dial.
//
// VASSALS IS THE GENTLE ONE and it is monotonic: 81% / 80% / 75% / 68% / 60%.
// Turning it off barely moves the human's own fortunes either way.
const UPKEEP_CHOICES = [
  [0, 'None', 'Nothing to keep. Build freely — and so will they.'],
  [0.5, 'Light', 'Half the bill.'],
  [1, 'Standard', 'What every figure in the game is balanced against.'],
  [1.5, 'Heavy', 'Half again. Fewer things stay built.'],
  [2, 'Punishing', 'Double. A long, attritional game.']
];
...
function drawSetup() {
  ...
  $('setup').innerHTML = `
    ...
    <div class="fld"><label>Upkeep — what you have built</label>
      <div class="opts trio">${UPKEEP_CHOICES.map(([v, n]) =>
        `<button class="opt${setup.upB === v ? ' on' : ''}" data-ub="${v}">${v}×${
          dfltMark(v === 1)}</button>`).join('')}</div>
      <p class="note" style="margin-top:10px"><b>${esc(UPKEEP_CHOICES.find(c => c[0] === setup.upB)[1])}.</b>
      ${esc(UPKEEP_CHOICES.find(c => c[0] === setup.upB)[2])}
      ${setup.upB === 1 ? '' : setup.upB < 1
        ? `<br><br>Cheaper buildings mean more money in the room and bigger rents, so the
           takeover arrives <b>more</b> often, not less — and at <b>0×</b> it turns against
           you hardest of all, because the opponents build everything and charge you for it.`
        : `<br><br>A dearer bill destroys money and slows everything down: fewer things stay
           built, fewer players are bankrupted into one another, and more games run to the
           circuit limit instead of ending in a takeover.`}</p>
    </div>
    <div class="fld"><label>Upkeep — vassals you hold</label>
      <div class="opts trio">${UPKEEP_CHOICES.map(([v, n]) =>
        `<button class="opt${setup.upV === v ? ' on' : ''}" data-uv="${v}">${v}×${
          dfltMark(v === 1)}</button>`).join('')}</div>
      <p class="note" style="margin-top:10px"><b>${esc(UPKEEP_CHOICES.find(c => c[0] === setup.upV)[1])}.</b>
      ${setup.upV === 0 ? 'Holding somebody costs nothing at all.'
        : 'What it costs to hold somebody, charged by how many you hold rather than per head.'}
      ${setup.upV === 1 ? '' : `<br><br>A gentler dial than the buildings one — it moves the
        takeover by about ten points across its whole range and leaves your own chances
        much where they were.`}</p>
    </div>
  ...
  $('setup').querySelectorAll('[data-ub]').forEach(b => b.onclick = () => {
    setup.upB = +b.dataset.ub; drawSetup();
  });
  $('setup').querySelectorAll('[data-uv]').forEach(b => b.onclick = () => {
    setup.upV = +b.dataset.uv; drawSetup();
  });
  $('begin').onclick = begin;
...
function begin() {
  ...
  G = E.createGame({ seats, seed: (Date.now() ^ (Math.random() * 1e9)) >>> 0,
    ...
    upkeepBuildings: setup.upB, upkeepVassals: setup.upV });
  ...
  $('acts').innerHTML = primary
    ...
    + actBtn('End turn', 'act_end()', G.phase === 'end' && !owesRoll ? 'pri' : '',
        owesRoll ? 'after the doubles roll'
          : E.upkeep(G, p) ? `upkeep ${money(E.upkeep(G, p))}` : '&nbsp;',
  ...
  switch (G.phase) {
    case 'roll':
      if (p.kind === 'ai') { busy = true; render(); later(act_roll, 750); }
        ...
        // render() runs at the top of tick, BEFORE this decides `busy` — so the
        // bar underneath was still drawn as "…is deciding" from the opponent's
        // turn, and the digest returning early meant nothing ever redrew it.
        // Acknowledging revealed a dead action bar. Draw it again now that busy
        // is known; the sheet lives in its own root and is untouched by this.
        busy = false;
        if (showDigest(p)) { render(); return; }
      }
      break;
...
const ACTIONS = {
  ...
  closeSheet, newGame, copyResult, act_payFee, act_pardon, showMenu, showGuide, toggleScore, showPlayer,
  ...
  setTithe, release, declare, tradeSet, sendTrade, takeCounter, counterOffer, offerFor, playOn
};
...
// The guide. EVERY FIGURE IN IT IS DERIVED, not typed -- which is the only
// reason it is safe to write at all. A guide is the single most likely thing in
// a game to go stale, and this session alone found three pieces of copy that
// had: a tithe sheet quoting the whole upkeep bill as the cost of one vassal, a
// settle sheet describing a debt spiral that two builds had already capped and
// repaid, and a citadel price written out in six places and wrong in all six.
// Nothing here restates a rule; copy.test.mjs fails the file if it starts to.
//
// It says what is OPTIONAL as well as what is true, because three settings now
// change the answers and a guide that ignored them would be wrong on any table
// that used them.
function showGuide() {
  const p = E.current(G);
  const r = G.rates || RULES;
  const g = r.garrison ?? RULES.garrisonUpkeep, cit = r.citadel ?? RULES.citadelUpkeep;
  const vas = (r.vassal ?? RULES.vassalUpkeep);
  const h = (title, body) => `<div class="sub" style="margin:18px 0 6px">${title}</div>
    <p style="font-size:14px;color:var(--dim);line-height:1.55;margin:0">${body}</p>`;
  sheet(`<h3>How this is played</h3>
    <div class="sub">circuit ${G.circuit} of ${G.circuits}</div>
    <p style="font-size:14px;color:var(--dim);line-height:1.55">Roll, settle where you land,
    then build or deal before you end the turn. Whoever holds the most when the swarm
    arrives has held the most — unless somebody takes everyone else first.</p>

    <div class="sub" style="margin:18px 0 6px">The buttons</div>
    <div class="stat"><span>Roll</span><span>starts your turn</span></div>
    <div class="stat"><span>Amend</span><span>${money(E.amendCost(p))} · nudge one square</span></div>
    <div class="stat"><span>Manage</span><span>build, pledge, redeem</span></div>
    <div class="stat"><span>Propose</span><span>draw a contract</span></div>
    <div class="stat"><span>End turn</span><span>pays your upkeep</span></div>
    ${h('Amend', `Moves you ONE square forward, before the square you are on is settled —
      so you may look at where you would land and nudge again, or stop. ${RULES.amendsPerGame}
      a game, at ${RULES.amendCosts.map(money).join(', then ')}. It is worth it in front of a
      citadel and almost nowhere else. Opponents use it on the same rule, each with its own
      idea of when the saving justifies the fee.`)}
    ${h('Contracts', `Up to ${RULES.tradeMax} squares each way plus credits, either direction.
      Nothing built or pledged may move, and neither side may hold a debt marker. Refuse an
      offer and you may <b>counter</b> it — their answer to that is the last word. When they
      refuse you, they will often name their own price.`)}
    ${h('Building', `You must hold every square of a colour to build on it. A garrison costs
      the set's price and keeps for ${money(g)} a turn; a citadel replaces three garrisons,
      costs one more garrison's price, and keeps for ${money(cit)}. Selling anything back
      returns half of what it cost. Garrisons and citadels are finite —
      ${G.garrisonPool} and ${G.citadelPool} are left.`)}
    ${h('Pledging', `Raises half a square's list price against it. A pledged square earns
      nothing and cannot be built on, but still counts toward its colour for whoever holds
      it. Redeeming costs ${Math.round(RULES.mortgageRedeemNumerator
        / RULES.mortgageRedeemDenominator * 100)}% of list — half back plus the interest.`)}
    ${h('When you cannot pay', `You are never removed from the table. Short against the bank
      becomes a debt marker at ${Math.round(RULES.debtInterest * 100)}% a turn${
      RULES.debtCap ? `, capped at ${money(RULES.debtCap)}` : ''}${
      RULES.lapRepaysDebt ? `, and a lap of the board pays ${money(RULES.passGo)} of it off`
        : ''}. While one stands you cannot buy, build, trade or bid. Short against a
      <b>player</b> is different: you are sworn to them instead.`)}
    ${h('Vassalage', `An overlord takes a share of everything you hold toward their total —
      you set the rate, ${RULES.titheRates.map(x => x + '%').join(' / ')} — and pays for the
      arrangement every turn: ${vas.map(money).join(' / ')} by how many they hold, not per
      head. Every turn under a tithe arms the vassal, and at enough strength they declare
      independence. A rival may seal a claim to take a vassal from you; the higher claim pays
      it to the bank, and a tie holds for whoever has them.`)}
    ${h('Detention', `Roll doubles to walk out, or after ${RULES.facilityAttempts} attempts
      pay ${money(RULES.facilityFee)}. A favour, if you hold one, spends instead. Three
      doubles in a row on one turn sends you there.`)}
    ${h('The swarm', `The clock. The deep array reports as it closes, and when it arrives the
      game is scored on totals — cash, squares and the share your vassals bring. Take every
      other player first and it ends before then.`)}

    <div class="sub" style="margin:18px 0 6px">This table</div>
    <div class="stat"><span>Dealt to each seat</span><span>${money(G.startingCash ?? RULES.startingCash)}</span></div>
    <div class="stat"><span>Neutral Anchorage</span><span>${G.anchoragePot
      ? 'holds the unclaimed tribute' : 'pays nothing'}</span></div>
    <div class="stat"><span>Upkeep</span><span>buildings ${G.upkeepBuildings ?? 1}× · vassals ${G.upkeepVassals ?? 1}×</span></div>
    <p style="font-size:13px;color:var(--dim);line-height:1.5;margin-top:8px">
    These three are chosen at setup and change the figures above. Everything else in the
    game is balanced against the defaults.</p>
    ${btns([['Back', 'showMenu', 'pri wide']])}`);
}
...
function showMenu() {
  ...
  sheet(`<h3>The ledger</h3><div class="sub">circuit ${G.circuit} of ${G.circuits}</div>
    ...
    ${btns([
      ...
      ['How this is played', 'showGuide', 'wide'],
    ...
    Saved after every move. Closing the app does not lose it; only starting a
    new one does.</p>
    <!-- A COLOPHON, not a bug-report form. Every figure here earns its place --
         the version answers "did the fix land", the seed replays the same dice
         and the same opponents, and the rest says which of the games on offer
         this one is. What it does NOT need is three lines explaining that to
         somebody who reports by screenshot, which is how this has been played
         all along. Reported as reading like a playtest artefact, and it did.
         The id and the shape of "Build <version> · seed <n>" are load-bearing:
         two probe checks read this element. -->
    <p style="font-size:12px;color:var(--dim);line-height:1.7;margin-top:16px;
       font-variant-numeric:tabular-nums;border-top:1px solid var(--rule);
       padding-top:12px" id="buildLine">
    <span style="letter-spacing:.14em;text-transform:uppercase;font-size:10px">This game</span><br>
    ${G.circuits} circuits · ${money(G.startingCash ?? RULES.startingCash)} a seat${
      G.anchoragePot ? ' · anchorage pot' : ''}${
      (G.upkeepBuildings ?? 1) !== 1 ? ` · buildings ${G.upkeepBuildings}×` : ''}${
      (G.upkeepVassals ?? 1) !== 1 ? ` · vassals ${G.upkeepVassals}×` : ''}<br>
    Build <b>${esc(BUILD)}</b> · seed <b>${G.seed >>> 0}</b></p>`);
}
...
function showClaim() {
  ...
  // THREE PLAYERS ARE INVOLVED and the sheet used to name none of them in its
  // figures: "Their holdings ₡540" meant the VASSAL's, not the rival's, and
  // "Your cash" sat under it as if the two were a comparison. Sam read it as
  // saying nothing about either side, which is the correct reading of it.
  //
  // So every row now names whose it is, and the three questions a claim
  // actually turns on are answered: what the prize is worth TO YOU (the tithe
  // share, not the vassal's gross holdings), what it will cost you every turn
  // to hold them, and how the two of you stand on vassals — which matters
  // because the bill is charged by count and because holding them to the end
  // is the conquest ending.
  //
  // WHAT IS DELIBERATELY NOT HERE is the rival's purse. The trade sheet shows a
  // counterparty's cash and should, because there you are pricing an offer
  // somebody must accept. A sealed bid is the opposite: knowing they hold ₡120
  // turns the whole mechanic into "bid ₡121", and the guessing IS the mechanic.
  // Their vassal count is public on the chips already; their cash is not.
  const rival = G.players[incumbent ? c.claimant : c.incumbent];
  const share = Math.floor(E.holdingsValue(v) * p.tithe / 100);
  // The incumbent is already paying the step at their current count; a
  // challenger would be paying the step at one above it. Same arithmetic, asked
  // from opposite ends -- see vassalStep in engine.js.
  const cost = E.vassalStep(G, p.vassals.length + (incumbent ? 0 : 1));
  sheet(`<h3>Competing claim</h3>
    <div class="sub">for ${esc(v.name)} · ${esc(p.name)} only</div>
    ${c.queue.length > 1 ? `<div class="warnbox"><b>Pass the phone to ${esc(p.name)}.</b>
      Neither claim is seen until both are in.</div>` : ''}
    <p style="font-size:14.5px;line-height:1.5;color:var(--dim)">
    ${incumbent
      ? `You hold ${esc(v.name)}. <b>${esc(chipName(rival))}</b> is trying to take them from you.`
      : `${esc(v.name)} cannot settle with you — and <b>${esc(chipName(rival))}</b> already holds their oath.`}
    Sealed and once each. The higher claim pays it <b>to the bank</b> and takes the vassal;
    a tie holds for ${incumbent ? 'you' : esc(chipName(rival))}, who ${incumbent ? 'hold' : 'holds'} them now.</p>

    <div class="sub" style="margin:16px 0 6px">What is at stake</div>
    <div class="stat"><span>${esc(chipName(v))} holds</span><span>${money(E.holdingsValue(v))}</span></div>
    <div class="stat"><span>Worth to you at ${p.tithe}%</span><span>${money(share)}</span></div>
    <div class="stat"><span>${incumbent ? 'Costing you' : 'Would cost you'}</span>
      <span>${money(cost)}/turn</span></div>

    <div class="sub" style="margin:16px 0 6px">Where you stand</div>
    <div class="stat"><span>Your cash — all you may claim</span><span>${money(p.cash)}</span></div>
    <div class="stat"><span>You hold</span><span>${p.vassals.length} vassal${p.vassals.length === 1 ? '' : 's'}</span></div>
    <div class="stat"><span>${esc(chipName(rival))} holds</span>
      <span>${rival.vassals.length} vassal${rival.vassals.length === 1 ? '' : 's'}</span></div>
    <p style="font-size:13px;color:var(--dim);line-height:1.5;margin-top:8px">
    You cannot see what they will claim. Whatever you seal is gone whether you win or not.</p>
...
// Holdings arranged by colour set, because a set is the unit that changes rent,
// and "2 of 3" is the thing you are actually reading for. Fleets, utilities and
// anything setless fall to the end under their own headings.
//
// One implementation on purpose. The player sheet grouped its list and the
// Holdings sheet you actually build from did not, which is how "group by
// colour" came back as a request for a thing that already half existed.
// Returned in a fixed order so two sheets cannot disagree about it either.
...
function showManage() {
  ...
  // Grouped by set, the same way and in the same order as the player sheet --
  // see groupHoldings. The headings sit BETWEEN rows and never inside one:
...
function refreshManage() {
  ...
  const parts = [];
  // The GAME's rates, which a multiplier may have moved away from the rules.
  const r = G.rates || RULES;
  if (g) parts.push(`${g}×${money(r.garrison ?? RULES.garrisonUpkeep)}`);
  if (c) parts.push(`${c}×${money(r.citadel ?? RULES.citadelUpkeep)}`);
  if (p.vassals.length) parts.push(`${p.vassals.length} vassal${p.vassals.length === 1 ? '' : 's'}`);
  set('[data-mg="upkeep"]', money(E.upkeep(G, p)) + (parts.length ? ` · ${parts.join(' + ')}` : ''));
...
function showTithe() {
  ...
  sheet(`<h3>Tithe rate</h3>
    <div class="sub">${p.vassals.length} vassal${p.vassals.length > 1 ? 's' : ''}
      · ${money(E.vassalUpkeep(G, p))}/turn of your ${money(E.upkeep(G, p))} upkeep</div>
    ...
    The vassal line is ${money(E.vassalUpkeep(G, p))} of your ${money(E.upkeep(G, p))} upkeep;
    the rest is what you have built and goes on costing it. Letting one go saves
    ${money(E.releaseSaving(G, p))} a turn${p.vassals.length > 1
...
function setTithe(r) {
  ...
  if (sub) sub.textContent =
    `${p.vassals.length} vassal${p.vassals.length > 1 ? 's' : ''} · upkeep ${money(E.upkeep(G, p))}/turn`;
}
...
function showContract() {
  ...
  sheet(`<h3>${esc(from.name)} → ${esc(to.name)}</h3>
    ...
    ${btns([['Accept', 'answerContract|1', 'pri'],
            ['Counter', 'counterOffer', ''],
            ['Refuse', 'answerContract|0', 'dgr']])}`);
}
...
// COUNTERING AN OFFER, which an opponent could do and a player could not.
// When a human proposes and an opponent refuses, aiCounter names a price back
// and the player takes it or walks. The other direction had no reply at all: an
// offer you did not like could only be accepted or refused.
//
// This is a refusal followed by a proposal of your own, not a new mechanic. The
// refusal is recorded exactly as any other, so an opponent still learns from
// being told no, and the counter goes through proposeContract and the same
// acceptance test everything else does.
//
// MIRRORED, because TR is written from the proposer's side and this contract was
// written from theirs. What they offered to give you is what you now ask to GET;
// what they wanted is what you now offer to GIVE. The cash flips with it:
// direction 1 means `from` pays `to`, so an offer where they were paying becomes
// a proposal where they still are -- which is -1 once you are the one proposing.
//
// It terminates on its own rather than needing a cap. They offer, you counter,
// and their answer is the last word: accept, or refuse and name a price you can
// take or walk away from. One reply each, which is what they get from you.
function counterOffer() {
  const c = G.contract;
  if (!c) return;
  const me = G.players[c.to], them = G.players[c.from];
  closeSheet();
  E.respondToContract(G, false);
  TR = {
    from: me.i, to: them.i,
    get: E.sqList(c.give),          // what they were offering, now what you ask for
    give: E.sqList(c.get),          // what they wanted, now what you offer
    cash: c.cash || 0,
    direction: c.direction === 1 ? -1 : 1
  };
  drawTrade();
}
...
// `busy` is the flag that draws the bar as "…is deciding" while somebody else
// acts. Exposed because the digest bug only exists when it is still TRUE as the
// human's turn opens — a probe that sets up the turn by hand never reaches that
// state, so the check for it passed with the fix removed.
window.__busy = v => { if (v !== undefined) busy = v; return busy; };
window.__COL = () => COLUMN;
