...
export const BUILD = 'grandiose-v110';
...
export const RULES = {
  ...
  amendsPerGame: 3,
  // Halved from 10 on Sam's call, and the citadel taken to 20 rather than 15
  // because he wanted a pair of round numbers. Measure with upkeep.mjs, which
  // carries both tables and the caveats; the short version:
  //
  // The defect was not that buildings were dear, it was that THE FIRST GARRISON
  // DID NOT PAY FOR ITSELF ON ANY SET ON THE BOARD -- from -5 a turn on Agora
  // to -26 on Enigma, at three opponents. The first rung of the ladder was a
  // tax, and 70% of the garrisons built in a game were torn down again. At 5
  // three sets pay from the first garrison and two more are within 3 of it.
  //
  // What it costs is the same thing the vassal bill cost, for the same reason:
  // a credit not destroyed by upkeep stays in the room and becomes rent, and
  // rent concentrates where upkeep merely destroys. 1000 games at 96 circuits
  // on Sam's table: conquest 50% -> 77%, and the human ends the game sworn to
  // somebody 36% -> 51%. About two points of conquest per point of being
  // sworn, in a straight line -- there is no rate that buys one without the
  // other, and no sweet spot to find. This is a chosen place on that line.
  //
  // NOTE THE RATIO. At 10/30 a citadel cost exactly what the three garrisons it
  // replaces cost, so upgrading was upkeep-neutral. At 5/20 it costs 5 a square
  // MORE, so a citadel now raises your bill as well as your rent. It is still
  // worth building everywhere (+18 to +95 a turn at three opponents), but
  // anything reasoning about the ladder must derive that step from both
  // numbers rather than assuming it is free -- upkeep.mjs assumed it, and was
  // wrong the moment these two stopped being 1:3.
  garrisonUpkeep: 5,             // per garrison, per turn
  citadelUpkeep: 20,             // per citadel, per turn
